...
page_init(const struct bootinfo *bi)
{
	...
	for (size_t i = 0, j = 0; i < bi->nr_rams; ++i) {
		...
		if (!page_reserve(r, r.base, r.begin - r.base, PG_HOLE))
			panic("bad bootinfo");
		if (!page_reserve(r, r.end, r.base + r.size - r.end, PG_HOLE))
			panic("bad bootinfo");
	...
	for (size_t i = 0; i < bi->nr_rams; ++i) {
		...
		switch (m->type) {
		case MT_MEMHOLE:
			if (!page_reserve((phys*)m->base, m->size, PG_HOLE))
				panic("bad bootinfo");
			break;
		case MT_KERNEL:
		...
		case MT_BOOTDISK:
			...
			break;
		case MT_RESERVED:
			if (!page_reserve((phys*)m->base, m->size, PG_FIXED))
				panic("bad bootinfo");
			break;
		}
	}
	...
	if (m_alloc > r_alloc->begin) {
		...
		if (!page_reserve(begin, m_alloc - begin, PG_SYSTEM))
			panic("bug");
	}
